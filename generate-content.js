...
/**
 * Content Generation Script for AxumPulse API with Thumbnail Generation
 * 
 * This script:
 * 1. Logs in as trainers using provided credentials
 * 2. Processes workout videos from the source directory
 * 3. Generates thumbnails automatically using FFmpeg (if available)
...
// Content templates for randomization
...
class ContentGenerator {
    ...
    /**
     * Upload file to API (video or thumbnail)
     */
        ...
        try {
            ...
            for (const videoFile of selectedVideos) {
                try {
                    ...
                    // Step 2: Create content record first (without file URLs)
        ...
        for (const credentials of TRAINER_CREDENTIALS) {
            try {
                const results = await this.generateContentForTrainer(
                    ...
                    8 // Generate 3 videos per trainer
                );
                allResults.push({
...
// Main execution
async function main() {
    console.log('🎬 AxumPulse Content Generator');
    console.log('==============================');
    ...
    if (TRAINER_CREDENTIALS.length === 0) {
        ...
        console.log('Example:');
        console.log('const TRAINER_CREDENTIALS = [');
        console.log('    { phone: "+251XXXXXXXXX", password: "password" },');
        console.log('    { phone: "+251XXXXXXXXX", password: "password" },');
        console.log('];');
        return;
    }
